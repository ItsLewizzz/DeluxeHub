...
public class AutoBroadcast extends Module implements Runnable {
	...
	public void onEnable() {
		...
		if(size > 0) broadcastTask = Bukkit.getScheduler().scheduleSyncRepeatingTask(getPlugin(), this, 60L, config.getInt("announcements.delay") * 20);
	}
		...
		if (count < size) {
			for(Player player : Bukkit.getOnlinePlayers()) {
				broadcasts.get(count).forEach(message -> {
					if(message.contains("<center>") && message.contains("</center>")) message = TextUtil.getCenteredMessage(message);
